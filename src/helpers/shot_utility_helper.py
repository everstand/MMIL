# -*- coding: utf-8 -*-
"""Shot-level pseudo utility helper for optional listwise MIL training.

This module is used only when `--rank-loss listwise_utility` is enabled.
It never reads human labels and never constructs a teacher from gtscore/user_summary.
...
The helper recomputes utility formulas from stored components so that the trainer
can switch formulas without regenerating the npy file.
"""
